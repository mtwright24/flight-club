-- If 20260430250000 / 20260430250100 ran while Robert was missing, their DO blocks exited
-- early and no rows were inserted — but those migrations are still marked applied.
-- This migration re-applies the full five-scenario seed when Marsha + Robert exist and
-- fewer than five staff_loads_demo requests are present (idempotent when already complete).
...
begin
  if not exists (select 1 from auth.users where id = v_marsha) then
    raise notice 'staff_loads_demo_repair: skip — Marsha % not in auth.users', v_marsha;
    return;
  end if;
  ...
  if v_robert is null then
    raise notice 'staff_loads_demo_repair: skip — no Robert user (add handle robert/bob/rwalker or robert%%@ email)';
    return;
  end if;

  if v_robert = v_marsha then
    raise notice 'staff_loads_demo_repair: skip — Robert resolved to same id as Marsha';
    return;
  end if;
  ...
  if v_cnt >= 5 then
    raise notice 'staff_loads_demo_repair: skip — % demo requests already present', v_cnt;
    return;
  end if;
  ...
  raise notice 'staff_loads_demo_repair: applied full seed (5 requests) for Marsha % / Robert %', v_marsha, v_robert;
end
$repair$;

-- Staff Loads: Marsha credits + demo reseed fixes.
-- 1) Marsha user_credits balance >= 25, priority_balance >= 10; profiles.credits_balance >= 5.
-- 2) staff_loads_reseed_demos: resolve responder as robert/bob/rwalker OR first other auth user; if still alone, seed 4 minimal Marsha-only demo rows.
-- 3) rpc_staff_loads_dev_reseed_demos: Marsha can trigger reseed from the app.
...
insert into public.user_credits (user_id, balance, priority_balance, updated_at)
values ('85f152bb-4b50-44c6-9f31-74f5906abb38'::uuid, 25, 10, now())
on conflict (user_id) do update set
  balance = greatest(public.user_credits.balance, 25),
  priority_balance = greatest(coalesce(public.user_credits.priority_balance, 0), 10),
  updated_at = now();
...
update public.profiles
set credits_balance = greatest(coalesce(credits_balance, 0), 5)
where id = '85f152bb-4b50-44c6-9f31-74f5906abb38'::uuid;
...
begin
  ...
  if auth.uid() <> '85f152bb-4b50-44c6-9f31-74f5906abb38'::uuid then
    return jsonb_build_object('ok', false, 'reason', 'forbidden', 'hint', 'Only the Marsha demo account can call this RPC.');
  end if;
...
comment on function public.rpc_staff_loads_dev_reseed_demos() is
  'Dev: Marsha-only — re-run Staff Loads demo seed from the app.';

-- Marsha May 2026 full FLICA demo: legs + layover text + DEPL/ARRL + flt notes + PTV + month strip.
-- trip_group_id matches schedule_trip_metadata (a1111111…) so pairing totals & crew merge.
-- month_key 2026-05 for all rows (Apr 29–30 show in May view).
...
begin
  if not exists (select 1 from auth.users where id = v_user) then
    raise notice 'reseed_marsha_may_2026_flica: skip — user % not found', v_user;
    return;
  end if;

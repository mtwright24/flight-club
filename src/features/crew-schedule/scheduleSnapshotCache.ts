...
export function readScheduleMonthUISnapshot(
  monthKey: string,
): ScheduleMonthUISnapshot | undefined {
  const v = monthUi.get(monthKey);
  if (!v) return undefined;
  if (tripListHasJ4195StaleMay292026(v.trips)) {
    logTripsForJ4195FakeMay29(
      v.trips,
      "scheduleSnapshotCache.monthUi.evict_on_read",
      "local_cache_only",
    );
...
export function purgeScheduleMonthUISnapshotsJ4195FakeMay292026(): void {
  for (const [k, snap] of [...monthUi.entries()]) {
    if (tripListHasJ4195StaleMay292026(snap.trips)) {
      logTripsForJ4195FakeMay29(
        snap.trips,
        "scheduleSnapshotCache.monthUi.purge",
        "local_cache_only",
      );
      monthUi.delete(k);
    }
...
export function writeScheduleMonthUISnapshot(
  snap: ScheduleMonthUISnapshot,
): void {
  monthUi.set(snap.monthKey, {
    ...snap,
    trips: JSON.parse(JSON.stringify(snap.trips)) as CrewScheduleTrip[],
    classicRows: JSON.parse(
      JSON.stringify(snap.classicRows),
    ) as ClassicScheduleRow[],
    monthMetrics: snap.monthMetrics
      ? JSON.parse(JSON.stringify(snap.monthMetrics))
      : null,
  });
}
...
/** Clears all in-memory classic/list UI snapshots (trips + duty-shaped classic rows). Does not touch Supabase or trips. */
export function clearAllScheduleMonthUISnapshots(): void {
  monthUi.clear();
}
 ...
 */
export function isScheduleMonthUISnapshotCoherent(
  snap: ScheduleMonthUISnapshot,
  year: number,
  month: number,
): boolean {
  if (!snap?.monthKey || snap.monthKey !== monthCalendarKey(year, month))
    return false;
  if (!Array.isArray(snap.trips) || !Array.isArray(snap.classicRows))
    return false;
  ...
  if (snap.trips.length > 0) {
    const anyOverlap = snap.trips.some((t) =>
      overlapMonth(t.startDate, t.endDate, ym, last),
    );
...
export function canSaveScheduleMonthUISnapshot(params: {
  ...
  if (
    !params.monthKey ||
    !Array.isArray(params.trips) ||
    !Array.isArray(params.classicRows)
  )
    return false;

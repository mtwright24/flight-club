...
import { extractLayoverRestFourDigits, parseScheduleTimeMinutes } from './scheduleTime';
import type { TripSummary } from './types';

export type TripSummaryPackExtra = {
  crew?: Array<{ position?: string | null; crew_name?: string | null; role_label?: string | null }>;
  hotels?: Array<{
...
function computeTafbMinutes(tripDuties: ScheduleDuty[], legsChrono: SchedulePairingLegLite[]): number {
  const firstIso =
    tripDuties.length > 0
  ...
  const lastEnd = (() => {
    ...
    if (!L) return parseScheduleTimeMinutes(tripDuties[tripDuties.length - 1]?.duty_off_time);
...
function buildRouteDash(legs: SchedulePairingLegLite[], tripDuties: ScheduleDuty[], base?: string | null): string {
  ...
  const sortedDuties = [...tripDuties].sort((a, b) => String(a.duty_date).localeCompare(String(b.duty_date)));
  for (const d of sortedDuties) {
    const c = String(d.layover_city ?? '').trim();
    if (c) push(c);
  }
  const hadLayovers = sortedDuties.some((d) => String(d.layover_city ?? '').trim());
...
function countLayoverNightsFromDuties(tripDuties: ScheduleDuty[]): number {
  const withStop = tripDuties.filter(
    (d) =>
      String(d.layover_city ?? '').trim().length > 0 || String(d.hotel_name ?? '').trim().length > 0,
  );
  return new Set(withStop.map((d) => sliceIso10(d.duty_date)).filter(Boolean)).size;
}
...
function pickHotel(
  ...
  if (hotels?.length) {
    const sorted = [...hotels].sort((a, b) => (Number(b.nights) || 0) - (Number(a.nights) || 0));
    const h = sorted[0]!;
  ...
  const d = tripDuties.find((x) => String(x.hotel_name ?? '').trim() || String(x.layover_city ?? '').trim());
...
export function buildTripSummaryFromNormalized(
  ...
  if (!dateOrder.length && legs.length) {
    dateOrder = [
      ...new Set(legs.map((l) => sliceIso10(l.duty_date)).filter((x): x is string => Boolean(x))),
    ].sort();
  }
  ...
  for (const dateIso of dateOrder) {
    ...
    const dep = String(first.departure_station ?? '').trim().toUpperCase().slice(0, 4);
    const arr = String(last.arrival_station ?? '').trim().toUpperCase().slice(0, 4);
    const duty = dutyByDate.get(dateIso);
    let dayBlock = 0;
    for (const L of dayLegs) dayBlock += legBlockHours(L);
    const report = formatTimeToken(duty?.report_time ?? first.scheduled_departure_local);
    const departStr = formatTimeToken(first.scheduled_departure_local) ?? '—';
    const arrStr = formatTimeToken(last.scheduled_arrival_local) ?? '—';
    const dutyEnd = formatTimeToken(last.release_time_local ?? last.scheduled_arrival_local);

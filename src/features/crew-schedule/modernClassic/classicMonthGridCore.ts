...
export type DayRow = {
  ...
  /**
   * When true, list/grid must show `pairingText` / `cityText` from FLICA mini-calendar only
   * (no leg-route substitution, no carry-line city override).
   */
...
function bucketClassicRowsByDate(
  rows: ClassicScheduleRow[],
): Map<string, ClassicScheduleRow[]> {
...
function sortClassicRowsSameDay(
  arr: ClassicScheduleRow[],
): ClassicScheduleRow[] {
  return [...arr].sort((a, b) => {
 ...
 */
function classicRowBlocksPtvTripOverlay(
  classic: ClassicScheduleRow | undefined,
): boolean {
  if (!classic) return false;
  if (isFlicaNonFlyingActivityId(String(classic.sourcePairingId ?? "")))
    return false;
...
function tripForDisplayDate(
  ...
  if (classic?.sourcePairingId) {
    const byPairing = findTripForDutyDay(
      mergedTrips,
      dateIso,
      classic.sourcePairingId,
    );
  ...
  const overlap = mergedTrips.filter(
...
export function attachDayRowGrouping(rows: DayRow[]): DayRow[] {
  for (let i = 0; i < rows.length; i += 1) {
    const cur = rows[i]!;
    if (cur.useFlicaLedgerLabels) {
      cur.groupedWithPrev = false;
      cur.groupedWithNext = false;
      continue;
    }
    const prev = rows[i - 1];
    const next = rows[i + 1];
...
export function buildClassicDisplayItems(
  ...
  for (const dateIso of carryInDates) {
    for (const c of sortClassicRowsSameDay(bucket.get(dateIso) ?? [])) {
      result.push({
        dateIso,
        classic: c,
        trip: tripForDisplayDate(mergedTrips, dateIso, c),
      });
    }
  }
  ...
  for (const dateIso of monthDates) {
    ...
    if (!list?.length) {
      result.push({
        dateIso,
        classic: undefined,
        trip: tripForDisplayDate(mergedTrips, dateIso, undefined),
      });
    } else {
  ...
  for (const r of carrySorted) {
    ...
    result.push({
      dateIso,
      classic: r,
      trip: tripForDisplayDate(mergedTrips, dateIso, r),
    });
  }
